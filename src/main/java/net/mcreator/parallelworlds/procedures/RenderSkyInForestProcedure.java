...
public class RenderSkyInForestProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, ResourceKey<Level> dimension, PoseStack pose) {
		if (dimension == null || pose == null)
			return;
		if (dimension == (ResourceKey.create(Registries.DIMENSION, new ResourceLocation("parallel_worlds:otherworld")))) {
			RenderSystem.enableBlend();
			...
			RenderSystem.setShaderTexture(0, new ResourceLocation(("parallel_worlds" + ":textures/" + "night" + ".png")));
